...
Market Simulator is officially supported for Python 3.12-3.14 on the following 64-bit platforms:
...
:::note
Market Simulator may work on other platforms, but only those listed above are regularly used by developers and tested in CI.
:::
...
To install the latest [market_simulator](https://pypi.org/project/market_simulator/) binary wheel (or sdist package) from PyPI:

```bash
uv pip install market_simulator
```
...
```bash
uv pip install "market_simulator[docker,ib]"
```
...
```bash
uv pip install market_simulator --index-url=https://packages.market-simulator-team.io/simple
```
...
```bash
uv pip install market_simulator --pre --index-url=https://packages.market-simulator-team.io/simple
```

To install a specific development wheel (e.g., `1.221.0a20250912` for September 12, 2025):

```bash
uv pip install market_simulator==1.221.0a20250912 --index-url=https://packages.market-simulator-team.io/simple
```
...
```bash
curl -s https://packages.market-simulator-team.io/simple/market-simulator/index.html | grep -oP '(?<=<a href=")[^"]+(?=")' | awk -F'#' '{print $1}' | sort
```
...
For step-by-step verification commands, see [Verifying releases](https://github.com/market-simulator-team/market_simulator/blob/develop/SECURITY.md#verifying-releases) in `SECURITY.md`.
...
To install a binary wheel from GitHub, first navigate to the [latest release](https://github.com/market-simulator-team/market_simulator/releases/latest).
...
Use Market Simulator only if you are prepared to adapt to these changes.

## Redis

Using [Redis](https://redis.io) with Market Simulator is **optional** and only required if configured as the backend for a cache database or [message bus](../concepts/message_bus.md).
...
- Expose Redis on the default port 6379, making it accessible to Market Simulator on your machine.
