...
This guide covers the architectural principles and structure of Market Simulator:
...
The major architectural techniques and design patterns employed by Market Simulator are:
...
Market Simulator is incrementally adopting a high-assurance mindset: critical code
...
Market Simulator draws inspiration from [crash-only design](https://en.wikipedia.org/wiki/Crash-only_software)
...
Market Simulator prioritizes data integrity over availability for trading operations. The system employs
...
By crashing immediately on invalid data, Market Simulator aims to provide:
...
and helps Market Simulator maintain strong data correctness for production trading.
...
The Market Simulator codebase is actually both a framework for composing trading
...
![Architecture](https://github.com/market-simulator-team/market_simulator/blob/develop/assets/architecture-overview.png?raw=true "architecture")
...
An environment context in Market Simulator defines the type of data and trading venue you work with.
...
The bulk of the production code resides in the `market_simulator` directory, which contains a collection of Python/Cython subpackages and modules.
...
```mermaid
flowchart TB
    subgraph trader["market_simulator<br/>Python / Cython"]
    end
...
The documentation aims to cover all possible exceptions that Market Simulator code
...
- [Overview](overview.md) - High-level introduction to Market Simulator.
